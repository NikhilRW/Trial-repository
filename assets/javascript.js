...
async function playPause2(_element,_index,_playlistTitle){
    if(_index<0){
        ...
        _element = songslist[anchors2.length-2];
        _index = anchors2.length-2;
    }
    else if(_index>anchors2.length-2){
    ...
    audio1.src = `Songs/${_playlistTitle}/${_element}.mp3`;
...
async function main(){
    ...
    for (let index = 0; index < anchors.length; index++) {
        ...
        let b = await fetch(`./Songs/${element.textContent}`)
        let response= await b.text();
        let div =  document.createElement("div")
        div.innerHTML = response;
        let anchors3 =div.getElementsByTagName("a"); 
        for (const iterator of anchors3) {
        ...
        <div class="flex flex-wrap relative over">
                    <div class="flex playCoverBox items-center  p-[20px]  flex-col bg-[rgb(24,24,24)] hover:bg-[rgb(36,36,36)] w-[17vw] h-[48vh] object-contain rounded-[10px]">
                        ...
                        <img width="160" height ="160" class="playCover shadow1 w-[13vw] h-[28vh] mb-[10px] object-contain rounded-[10px]" src="Songs/${element.textContent}/${playListCoverSrc}">
                        <span class="flex flex-col justify-start w-[100%] ml-[9%]">
                            <p class="playlistTitle font-[Questrial] font-[600] text-left ">${element.textContent.replace("/","")}</p>
                            <p class="playlistDesc font-[Questrial] font-[12px] overflow-clip">Best Of ${element.textContent.replace("/","")} </p>
                        </span>
                    </div>
                </div>
        `
    ...
    for (let index = 0; index < playlistDivs.length; index++) {
...
async function fetchPlaylistsSongs(playlistTitle){
    let a = await fetch(`./Songs/${playlistTitle}`)
    let response= await a.text();
    let div =  document.createElement("div")
    div.innerHTML = response;
